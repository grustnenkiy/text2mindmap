mindmap = (function () {
    ...
    var I = function (q, r) {
        ...
        $(window).on("mouseup touchend", function (a) {
            ...
            t = s = m = !1;
            var positions = JSON.stringify(that.exportPositions());
            localStorage.setItem(positionsKey, positions);
            a.stopPropagation();
            a.preventDefault()
        });
        return that = {
            ...
            newRect: function (a, b, c) {
                return new Kinetic.Rect({
                    ...
                    height: c,
                    cornerRadius: 10,
                    fill: getBgColorByStatus(a.data.status),
                    stroke: 'black',
                    strokeWidth: 1
                })
            },
            newEllipse: function (a, b, c) {
            ...
            text2mindmap: function (a) {
                ...
                for (var c = difflib.stringAsLines(y), e = (new difflib.SequenceMatcher(c, a)).get_opcodes(), f = 0, m = 0, l = 0; l < e.length; l++)
                    for (var k = e[l], n = k[0], q = k[1], r = k[2], p = k[3], k = k[4], x = Math.max(r - q, k - p), v = 0; v < x; v++) {
                        if ("delete" == n || "replace" == n && p == k) {
                            ...
                            m++
                        } else "replace" == n && (s = q, t = H(a[p]), d.nodes[s].data.label = t.label, d.nodes[s].data.linkLabel = t.linkLabel, d.nodes[s].data.status = t.status);
                        q = q < r ? q + 1 : q;
                        p = p < k ? p + 1 : p
                    }
                var w = [],
                ...
                var positions = JSON.parse(localStorage.getItem(positionsKey));
                if (positions) {
                    that.importPositions(positions, 0.4);
                } else {
                    d.nodes.forEach(function (a, b) {
            ...
            setTheme: function (a) {
                a.font = e.font;
                a.fontColor = e.fontColor ? e.fontColor : "#000";
